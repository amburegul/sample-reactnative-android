...
export default function UpcomingScreen() {
    ...
    return (
        <View style={styles.container}>
            <View style={{ flex: 3 }}>
                <ScrollView>
                    <Image
                        ...
                        source={{ uri: ApiService.BASE_URL + petObj.photos }}
                    />
                    {/* <Text style={styles.h1}>{petObj.name.toUpperCase()}</Text> */}
                    {/* bio */}
                    <View style={styles.frame}>
                        <View style={{ paddingHorizontal: 15 }}>
                            <Text style={{ fontSize: fSize.h4, fontWeight: 'bold', fontStyle: 'italic' }}>NAMA</Text>
                            <Text style={{ fontSize: fSize.h5, marginLeft: 10 }}>{petObj.name}</Text>
                        </View>
                        <View style={{ borderBottomWidth: 1, marginVertical: 10 }} />

                        <View style={{ paddingHorizontal: 15 }}>
                            <Text style={{ fontSize: fSize.h4, fontWeight: 'bold', fontStyle: 'italic' }}>UMUR</Text>
                            <Text style={{ fontSize: fSize.h5, marginLeft: 10 }}>{petObj.age}</Text>
                        </View>
                        <View style={{ borderBottomWidth: 1, marginVertical: 10 }} />

                        <View style={{ paddingHorizontal: 15 }}>
                            <Text style={{ fontSize: fSize.h4, fontWeight: 'bold', fontStyle: 'italic' }}>JENIS</Text>
                            <Text style={{ fontSize: fSize.h5, marginLeft: 10 }}>{petObj.type}, ({petObj.breed})</Text>
                        </View>
                        {/* <View style={{borderBottomWidth:1, marginVertical:10}} /> */}
            ...
            <View style={{ flex: 2 }}>
                <Text style={{ fontSize: fSize.h3, color: colorTemplate[2], marginHorizontal: 15 }}>Riwayat Kunjungan</Text>
                {/* KUNJUNGAN */}
                <ExpandableFlatList />
            </View>
            {/* Floating Action Button (FAB) */}
            <TouchableOpacity style={styles.fab} onPress={() => router.push({pathname:"/(app)/(pets)/booking",params:{petObj:JSON.stringify(pet)}})}>
...
const ExpandableFlatList = () => {
    ...
    const loadVisits = async () => {
        try {
            const data = await ApiService.fetchVisitHistoryByPet(petObj.id, getToken());
            setVisits(data);
        } catch (error) {
    ...
    const renderItem = ({ item }: { item: Visit }) => (
        <View style={styles.itemContainer}>
            <TouchableOpacity onPress={() => toggleExpand(item.id.toString())} >
                <Text style={styles.title}>{item.visitDate} <FontAwesome name="angle-double-down" size={fSize.h5} /></Text>
            </TouchableOpacity>
            <View style={{ borderBottomWidth: 1, marginVertical: 10 }} />
